...
run_pamk <- function(dist) {
  ...
  structure(
    pamk(dist, krange = CLUSTERS, criterion = "ch", alpha = 0.001),
    dataset = attributes(dist)$dataset,
    algorithm = "pamk",
    metric = attributes(dist)$metric
  )
...
pamk_datasets <- names(object)[unique(map_int(pamk_results, ~ attributes(.x)$dataset))]
...
ac_values <- list(linkage = character(), ac = double())
...
# I would like to narrow to the top few metrics, but using the code above shows very little difference in agglomerative coefficient between metrics.
...
# I will do this with only Euclidean distance and correlation method for pvclust to save on computation time
...
pvclust_results <- c(
  ...
  imap(
    object,
    \(x, y) structure(pvclust(x, "ward.D2", "correlation"), dataset = y, method = "ward.D2", metric = "correlation")
  )
)
...
## Test (remove whole section later)
