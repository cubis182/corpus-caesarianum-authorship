...
all_vars_by_section_culled <- all_vars_by_section |>
  select_top_variables(n = 500, "sum") 
```
...
- Defaults in `get_dist()` ("euclidean", "maximum", "manhattan",
  ...
  "kendall".)
  - Euclidean can be challenging to use in high-dimensional vectors, but most vectors are going to be relatively close to each other here. In fact, this should make Euclidean distance one of the more ideal measures.
...
# I may avoid using this, since it requires a lot of setup that I have already done in the code blocks below. It really only would have saved time if I started with it, or used it on only a couple methods.
...
A few notes on each dataset: 
- Caesar has median 3 to 5 clusters; Gorman's stronger clustering tendency is responsible for the 5.
- The simplex features generate only two
clusters for all metrics except *cosine*, which gives 8 clusters. The
silhouette width is very small, though, for this 8-cluster one. 
- The same is true of the custom variable set, which equally has a very small
silhouette width - On Caesar alone, all datasets got similar values. How
similar? Let's visualize (NOTE: THIS IS OUT OF DATE, I WOULD NEED TO ADD COSINE BACK IN FOR THE FOLLOWING CODE TO WORK!):
...
- The Gorman variables are the highest-distinguishing group. See the
  following (NOTE: THIS IS OUT OF DATE, I WOULD NEED TO ADD COSINE BACK IN FOR THE FOLLOWING CODE TO WORK!):
...
```
Notes on the above dendrogram, **when performed with the simplex variables**:
- With Caesar alone, proceeding in order from the first linkage, the first statistically significant split is the link (au p-value = 97) between *BGall.* 8 and the rest of its cluster. The next significant result is the linkage between *BGall.* 2, 4 and *BGall.* 3, *BAlex* 1 (i.e. sections 1-22). The final significant result is the cluster combining *BHisp.* with the other members of the supplementary corpus, alongside some other curiosities.
- Notably, the split between the three main clusters is NOT significant (au p-value = 84). 
- A question of interpretation is what we say when a larger cluster is highly significant, but clusters lower in the tree are not. Does this mean that the clusters within the highly significant one link in different ways, but usually stay together?
