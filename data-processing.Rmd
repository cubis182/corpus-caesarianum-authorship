...
get_var_combos <- function(var_combn, source_data) {
  ...
  #browser()
  var_combos <- 
    source_data[selection_char]
  
  var_combos <- collapse::fcount(var_combos) |> na.omit()
...
transpose_all_vars <- function(all_vars) {
  ...
  duck_all_vars <- read_csv_duckdb(temp)
  
  duck_all_vars %<>% pivot_wider(
    all_vars, 
    names_from = pasted, 
    values_from = n,
    values_fill = 0)
...
select_top_variables <- function(all_vars, n) {
  ...
  all_vars_top_thousand <- all_vars[top_cols] |>
...
if (!("dtplyr" %in% (.packages()))) library(dtplyr)
  ...
  all_vars_by_book <- get_vars("book", data |> filter(path == 'amicitia'), presupplied_variables=NULL)
