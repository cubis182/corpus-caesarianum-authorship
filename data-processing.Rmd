...
data <- bind_rows(
  data,
  fread("./postagged/postagged-cicero.csv", na.strings = "", col.names = colnames(data), stringsAsFactors = TRUE)
  )
...
data %<>% tidytable::select(-NumType, if (params$use_parent) -parent_NumType else NULL, -Degree, if (params$use_parent) -parent_Degree else NULL) |>
  filter(tag != 'PUNCT')
...
Note that the YAML header params `new_combos` and `use_parent` affect the operation of the following code.
