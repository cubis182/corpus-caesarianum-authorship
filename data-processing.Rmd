...
# DEPRECATED CODE BLOCK, WILL REMOVE ONCE NEW STRATEGY IS IMPLEMENTED
...
# SPATIUM-L1
# SPATIUM is more than a measurement method; Kocher 2016 just uses Manhattan distance to compare vectors
...
Next, we try a variety of clustering algorithms. Which clustering method
should be used is another non-trivial problem. K-means has been expanded
many different ways. Note [@jain2010] p. 659 cites Fisher and vanNess on
the following requirements for a clustering algorithm to keep in mind:
...
# The input to this block is a set of `dist` objects from the previous code block's agglomerative clustering
