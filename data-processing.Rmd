...
- The top \~100 most common lemmas in Caesar. Each is combined with its tag, so
...
- Part of speech (POS) and POS n-grams in Caesar. POS n-grams encode the 
  number of times parts of speech occur in a row, so ADP –\> ADJ -\> NOUN 
  would be a 3-gram showing the number of times a preposition occurs before an
...
- Defaults in `get_dist()` ("euclidean", "maximum", "manhattan",
  ...
  "kendall".)
  - Pearson's r, which is correlation, not distance. More sensitive to outliers than Kendall or Spearman. 
  
  - Kendall's $\tau$, which is a correlation measure and rank-based, like Spearman.

  - Canberra distance, which gives smaller values more emphasis. Not as large a concern here.
...
- Spearman's $\rho$, which is a correlation measure and rank-based instead of linear.
...
The PCA analysis in the above cell seems to suggest that Gorman's method is still worth keeping. My custom variable set failed to show any cluster tendency when restricted to Caesar alone (see below), and it is the second-lowest here with the full corpus. I modified feature selection to focus only on Caesar instead of the full corpus, and changed from 50 to 100 lemmas, but neither improved performance here or the VAT diagram above. See Moisl and Stamatatos et al. 2009 p. 545 on the research that uses PCA for dimensionality reduction.
...
- The simplex features generate only two clusters for all metrics except *cosine*, which gives 8 clusters. The silhouette width is very small, though, for this 8-cluster one.
- The same is true of the custom variable set, which equally has a very small silhouette width
- On Caesar alone, all datasets got similar values. How similar? Let's visualize:
```{r}
...
```
- The Gorman variables are the highest-distinguishing group. See the following:
```{r}
...
print(v1$pamobject$silinfo)
```
- Canberra distance is shown below to result in a low agglomerative coefficient. 
